...
description: "The HDB Service dashboard surfaces real-time metrics — open tickets, SLA compliance, dispenser status breakdown, low-stock alerts, maintenance overdue counts, and MTTR analytics."
---
...
<ResponseField name="kpis" type="object">
  ...
  <Expandable title="Fields">
    <ResponseField name="sla" type="number">
      Average hours from ticket creation to first technician action (response to the repair or ticket resolution).
    </ResponseField>
...
| MTTR (hours to repair) | 20% | ≤ 24 hours |
| Maintenance condition rating | 20% | GOOD |
| Failure recurrence (last 6 months) | 15% | ≤ 2 events |
...
<ResponseField name="ranking" type="array">
  All dispensers in scope, sorted ascending by score (most critical first). Each entry includes `id`, `marca`, `modelo`, `serial`, `planta`, `score`, `status`, and a `details` block with raw `mtbfDays`, `mttrHours`, `recurrences`, `condition`, and `progress` values.
</ResponseField>
