...
description: "Track every water dispenser through installation, active service, repair, and retirement — with full location history, consumable records, and QR code identification."
---
...
| `status` | `DispenserStatus` | Current operational state (see below). Defaults to `BACKUP` on creation. |
...
- **`lifecycleExpiration`** — absolute date when the lifecycle ends, calculated as `lifecycleStartDate + (lifecycleMonths × 30 days) + lifecycleAccumulatedPauseDays`.
