...
`POST` endpoints support idempotent retries through an idempotency key mechanism implemented in `lib/idempotency.ts`. To use it, include an `Idempotency-Key` header containing a UUID with your request. If the server has already processed a request with that key and it succeeded, it will return the cached response immediately without re-executing the handler — preventing duplicate records or double side effects on network retries.
...
4. If no `Idempotency-Key` header is provided, the request is processed normally with no deduplication.
