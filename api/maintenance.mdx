---
title: "Maintenance API — Schedules, Checklists, and Approvals"
sidebarTitle: "Maintenance"
description: "REST API reference for maintenance scheduling: list schedules, generate monthly plans, submit checklists, complete visits, and verify digital signatures."
---
...
Creates a new maintenance approval record grouping one or more completed maintenance schedules under a single digital signature session. The approval is created with an empty signature and must be subsequently signed by the customer representative on the technician's device.
...
<ParamField body="scheduleIds" type="string[]" required>
...
Verifies the cryptographic integrity of a maintenance approval signature. The endpoint retrieves the stored `encryptedHash` and `hashPayload`, decrypts the hash with AES-256-GCM, recomputes the SHA-512 digest from the raw payload, and compares both using a timing-safe equality check.
...
<ResponseField name="computedHash" type="string">
