...
Returns a single user's full profile, including their plant access list. The `passwordHash` field is excluded from the response.
...
<ResponseField name="active" type="boolean">Whether the account is active.</ResponseField>
<ResponseField name="clientId" type="string">Associated client ID (nullable).</ResponseField>
<ResponseField name="mustChangePassword" type="boolean">Whether the user must change their password on next login.</ResponseField>
<ResponseField name="onesignalPlayerId" type="string">OneSignal device player ID for push notifications (nullable).</ResponseField>
<ResponseField name="createdAt" type="string">ISO 8601 creation timestamp.</ResponseField>
<ResponseField name="updatedAt" type="string">ISO 8601 last-updated timestamp.</ResponseField>
<ResponseField name="plantAccess" type="array">
  <Expandable title="plantAccess entry fields">
    <ResponseField name="plantId" type="string">Plant ID the user has access to.</ResponseField>
  </Expandable>
</ResponseField>

```bash
curl -X GET "https://your-domain.com/api/users/usr_abc123" \
  -H "Cookie: sb-access-token=<token>"
```
...
<ParamField body="onesignalPlayerId" type="string">OneSignal device player ID for push notification targeting.</ParamField>
